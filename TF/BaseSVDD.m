classdef BaseSVDD < handle & matlab.mixin.Copyable
    ...
    properties
        ...
        label
        cost = 0.9
        kernelFunc = Kernel('type', 'gaussian', 'gamma', 0.0000001)
        supportVectors
...
%            if (obj.alpha>1e-8)
% 
%             obj.SV_index =  obj.alpha
%            end
%            SV_value = obj.data(obj.alpha,:);
%             SV_alf = obj.alpha(obj.alpha);
%             r_index = obj.alpha(1);
%   term3 = sum(sum((obj.alpha *obj.alpha').*K));
% term4=25*sum(obj.alpha'*(K'*obj.data));
...
%           obj.radius = sqrt(mean(diag(K))+obj.offset+mean(tmp_(obj.boundary, :)))+sqrt(term3/800000);

         obj.radius = sqrt(mean(diag(K))+obj.offset+mean(tmp_(obj.boundary, :)))+sqrt(obj.offset/800000);
...
%            SV_threshold = 1e-8; 
%             SV_index = find(obj.alpha>1e-8);
%             if obj.alpha>1e-8
%               SV_index =  obj.alpha;
%             end
% SV_value = obj.data(SV_index,:);
% SV_alf = obj.alpha(SV_index);
%             r_index = SV_index(1,1);

% term1 = K(r_index,r_index);

% term2 = -2*K(r_index,:)*obj.alpha;

% term3 = sum(sum((obj.alpha *obj.alpha').*K));
% 
% term4=4*obj.alpha'*(K'*(obj.b*K)*obj.alpha);
% % radius
% obj.radius = term1+term2+term3+term4;
%             obj.radius =model.R;
